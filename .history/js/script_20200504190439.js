...
// const font_sizes = ['medium', 'large', 'x-large', 'xx-large'];
var   current_font_size = 'large';
...
function load_pages(book_name,language){  
  ...
  for (var i=0;i<storyObj.num_of_pages;i++){
    ...
    str += '<div id="content_text"><div class="story_text">' + text + '</div></div></div>';    
...
text_bigger_btn.addEventListener('click', function(){  
  ...
  //default font-size is 'large'
  //const font_sizes = ['medium', 'large', 'x-large', 'xx-large'];
  if (current_font_size==='medium'){
    current_font_size = 'large';
  } else if (current_font_size==='large'){
    current_font_size = 'x-large';
  } else if (current_font_size==='x-large'){
    current_font_size = 'xx-large'
  } else {
    //Biggest font size already set.
    return;
  }
  ...
  for (const elem of elements){
    ...
    var child = elem.children[0].children[1].children[0];
    console.log(child)
    var current_font_size = child.style['font-size'];
    console.log(current_font_size)
    var index = font_sizes.indexOf(current_font_size);
    if (index !== font_sizes.length-1){
      var new_font_size = font_sizes[index+1];
      console.log(new_font_size)
      child.style['font-size'] = new_font_size;
    }
  }
