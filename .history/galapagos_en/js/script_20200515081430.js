...
var orientation_change_in_process = false;

//Init
get_story_data(BOOKNAME, BOOK_LANG); 

//////////////////////////////////
function get_story_data(book_name, language){
  
  //Modify index.html for RTL
  if (language == "heb"){
  ...
  fetch("../assets/json/stories.json")
  .then(function(res){
    return (res.json());
  })
  .then(function(data) {
...
function load_pages(book_name,language){  
  ...
  // -File names are assumed to be - page4x3_#.jpg or page16x9_#.jpg
  // -Images folder should be organized as/book_name/4by3/page4x3_#.jpg (or similar for 16by9 )
  for (var i=0;i<storyObj.num_of_pages;i++){
    if (orientation_is_vertical){    
      var img_src =  "../assets/Images/" + book_name + "/" + language + "/4by3/page" + i +".jpg";
    } else {
      var img_src = "../assets/Images/" + book_name + "/" + language + "/16by9/page" + i +".jpg";
    }
  ...
  document.querySelector('#usage_notes_title').innerHTML       = storyObj.usage_notes.usage_notes_title;
  document.querySelector('#screen_timeout_subtitle').innerHTML = storyObj.usage_notes.screen_timeout_subtitle;
  document.querySelector('#usage_notes').innerHTML             = storyObj.usage_notes.usage_notes;
  ...
  audioElement = new Audio("../assets/Audio/" + book_name + "/" + language + "/page0.mp3"); 
  ...
  mySwiper.on('slideChange', function(){
    ...
    if (!orientation_change_in_process){ //We don't want to reload audio on orientation change.
      ...
      audioElement = new Audio("../assets/Audio/" + book_name + "/" + language + "/page" + mySwiper.activeIndex + ".mp3");
      audioElement.addEventListener('ended', function(){     
        mySwiper.slideNext();
      });
...
window.addEventListener("orientationchange", function(){
  ...
  //This bol is set to prevent the 'slidechange' event from
  //reloading the audio, which causes a pause-before-play race
  //condition and a DomException.
  orientation_change_in_process = true;
  ...
  for (var i=0;i<storyObj.num_of_pages;i++){
    if (orientation_is_vertical){    
      var img_src =  "../assets/Images/" + BOOKNAME + "/" + BOOK_LANG + "/4by3/page" + i +".jpg";
    } else {
      var img_src = "../assets/Images/" + BOOKNAME + "/" + BOOK_LANG + "/16by9/page" + i +".jpg";
    }
  ...
  orientation_change_in_process = false;  
})
